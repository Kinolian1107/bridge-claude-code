 ...
 * plugin tools (LSP) and MCP connector tools survive and would still run on the
 * bridge host. Verified against Claude Code 2.1.x (session init tool list):
 *   --tools ""                                          → LSP + mcp__* survive
 *   --tools "" --strict-mcp-config                      → LSP survives
 *   --tools "" --strict-mcp-config --disallowedTools LSP → empty (pure LLM)
 *
 * - `--strict-mcp-config` ignores every host MCP config (no `--mcp-config` is
 *   passed, so the effective MCP set is empty).
 * - `--disallowedTools LSP` denies the surviving LSP plugin tool by name.
 *
 * (`--bare` would also drop plugins/LSP, but it forces ANTHROPIC_API_KEY auth and
 * never reads OAuth/keychain, breaking subscription logins — so it is NOT used.)
 *
 * @returns {string[]} args to append to the `claude` invocation in llm mode
 */
export function llmHardeningArgs() {
  return ["--tools", "", "--strict-mcp-config", "--disallowedTools", ...LLM_DISALLOWED_TOOLS];
}
