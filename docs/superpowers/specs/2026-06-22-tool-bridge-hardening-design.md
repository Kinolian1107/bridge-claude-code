...
| 參數 | 事件序列 | 文字到達 |
|---|---|---|
| `stream-json --verbose`(**= 目前 bridge**) | `system×3 → assistant → rate_limit_event → result` | **單一 `assistant` 事件含整段文字**,無 `content_block_delta` |
...
- `result` 事件:`{type:"result", subtype, is_error, api_error_status, result:"<文字>", stop_reason, usage:{input_tokens, output_tokens, cache_creation_input_tokens, cache_read_input_tokens,...}}`。
...
- **串流 + toolBridgeMode**:建 `scanner`。assistant 事件**只取 `part.type==="text"`**(thinking guard,維持現有過濾)→ `scanner.push(text)`;`text`→content delta、`toolCalls`→逐一發 `{delta:{tool_calls:[{...,index}]}}`、`anomalies`→metrics+log。`proc.close`:`scanner.flush()` 同樣落地;`emittedAnyCall ? "tool_calls" : "stop"`;`recordToolCalls`。
...
- `createToolCallScanner`:純文字穿透;單區塊單 push;opener 跨界;closer 跨界;單 push 多區塊(index 0/1);文字夾區塊;巢狀參數;未閉合 flush→fallback+`unterminated`;fenced。
