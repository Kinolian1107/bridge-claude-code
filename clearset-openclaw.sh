...
# Remove bridge-claude-code integration from OpenClaw config
# ─────────────────────────────────────────────────────────────
...
echo ""
echo "┌──────────────────────────────────────────────────┐"
echo "│  bridge-claude-code → OpenClaw: remove integration│"
echo "└──────────────────────────────────────────────────┘"
echo ""
...
if [ -f "$BACKUP" ]; then
  ...
  if [[ -z "$ans_restore" || "$ans_restore" =~ ^[Yy] ]]; then
    ...
    info "Removing claude-cli provider from current config..."
    _remove_provider=true
  fi
else
  warn "No backup found — removing claude-cli provider from current config"
  _remove_provider=true
fi

# ── Remove claude-cli provider from config ───────────────────
...
// Clear default model reference if it points to claude-cli
