...
# ─────────────────────────────────────────────────────────────
# set-hermesagent.sh
# Configure Hermes Agent to use bridge-claude-code as the model provider
# ─────────────────────────────────────────────────────────────
...
echo ""
echo "┌──────────────────────────────────────────────────┐"
echo "│  bridge-claude-code → Hermes Agent integration    │"
echo "└──────────────────────────────────────────────────┘"
echo ""
...
info "Target model:    ${CLAUDE_MODEL}"
[ -n "$AVAILABLE_MODELS" ] && info "Available:       ${AVAILABLE_MODELS}"
echo ""

# ── Backup current config ────────────────────────────────────
BACKUP="${HERMES_CONFIG}.bak.pre-claude-bridge"
if [ -f "$BACKUP" ]; then
...
info "Configuring Hermes Agent to use bridge-claude-code..."
...
"$HERMES_BIN" config set model.default "${CLAUDE_MODEL}"
ok "Set model.default = ${CLAUDE_MODEL}"

echo ""
ok "Hermes Agent configured to use bridge-claude-code (model: ${CLAUDE_MODEL})"

# ── Sync all models to custom_providers ─────────────────────
echo ""
info "Syncing all bridge-claude-code models to Hermes custom_providers..."
...
base_url    = sys.argv[3]
PROVIDER_NAME = "bridge-claude-code"

with open(config_path, encoding="utf-8") as f:
    cfg = yaml.safe_load(f)

existing = cfg.get("custom_providers") or []
# Keep non-bridge-claude-code entries intact
...
new_entries = [
    {
        "name":     PROVIDER_NAME,
        "base_url": base_url,
        "api_key":  "claude-code-bridge-local",
